...
Desc_Skel = {}
Scoreboard = {
    f'{NAME_PREFIX}waiting': {'key': '_', 'desc': 'Waiting for Connection'},
    f'{NAME_PREFIX}starting': {'key': 'S', 'desc': 'Starting up'},
    f'{NAME_PREFIX}reading_request': {'key': 'R', 'desc': 'Reading Request'},
    f'{NAME_PREFIX}sending_reply': {'key': 'W', 'desc': 'Sending Reply'},
    f'{NAME_PREFIX}keepalive': {'key': 'K', 'desc': 'Keepalive (read)'},
    f'{NAME_PREFIX}dns_lookup': {'key': 'D', 'desc': 'DNS Lookup'},
    f'{NAME_PREFIX}closing': {'key': 'C', 'desc': 'Closing connection'},
    f'{NAME_PREFIX}logging': {'key': 'L', 'desc': 'Logging'},
    f'{NAME_PREFIX}gracefully_fin': {
        'key': 'G',
        'desc': 'Gracefully finishing',
    },
    f'{NAME_PREFIX}idle': {'key': 'I', 'desc': 'Idle cleanup of worker'},
    f'{NAME_PREFIX}open_slot': {
        'key': '.',
...
Metric_Map = {
    'Uptime': f"{NAME_PREFIX}uptime",
    'IdleWorkers': f"{NAME_PREFIX}idle_workers",
    'BusyWorkers': f"{NAME_PREFIX}busy_workers",
    'Total kBytes': f"{NAME_PREFIX}bytes",
    'CPULoad': f"{NAME_PREFIX}cpuload",
    "Total Accesses": f"{NAME_PREFIX}rps",
}
...
def get_metrics():
    ...
    if (time.time() - METRICS['time']) > METRICS_CACHE_MAX:
        ...
        try:
            req = urllib2.Request(f"{SERVER_STATUS_URL}?auto")
        ...
        if COLLECT_SSL:
            ...
            try:
                ...
                for line in res:
                    if regMatch := SSL_REGEX.match(line):
...
def metric_init(params):
    ...
    descriptors.append(
        create_desc(
            {
                "name": f"{NAME_PREFIX}busy_workers",
                "value_type": "uint",
                "units": "threads",
                "format": "%u",
                "call_back": get_value,
    ...
    descriptors.append(
        create_desc(
            {
                "name": f"{NAME_PREFIX}idle_workers",
                "value_type": "uint",
                "units": "threads",
                "format": "%u",
                "call_back": get_value,
    ...
    if params['collect_ssl']:
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}shared_mem",
                    "value_type": "float",
                    "units": "bytes",
                    "format": "%.3f",
                    "call_back": get_value,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}current_sessions",
                    "value_type": "uint",
                    "units": "sessions",
                    "format": "%u",
                    "call_back": get_value,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}num_subcaches",
                    "value_type": "uint",
                    "units": "subcaches",
                    "format": "%u",
                    "call_back": get_value,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}indexes_per_subcache",
                    "value_type": "float",
                    "units": "indexes",
                    "format": "%.3f",
                    "call_back": get_value,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}index_usage",
                    "value_type": "float",
                    "units": "pct",
                    "format": "%.3f",
                    "call_back": get_value,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}cache_usage",
                    "value_type": "float",
                    "units": "pct",
                    "format": "%.3f",
                    "call_back": get_value,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}sessions_stored",
                    "value_type": "float",
                    "units": "sessions/sec",
                    "format": "%.3f",
                    "call_back": get_delta,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}sessions_expired",
                    "value_type": "float",
                    "units": "sessions/sec",
                    "format": "%.3f",
                    "call_back": get_delta,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}retrieves_hit",
                    "value_type": "float",
                    "units": "retrieves/sec",
                    "format": "%.3f",
                    "call_back": get_delta,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}retrieves_miss",
                    "value_type": "float",
                    "units": "retrieves/sec",
                    "format": "%.3f",
                    "call_back": get_delta,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}removes_hit",
                    "value_type": "float",
                    "units": "removes/sec",
                    "format": "%.3f",
                    "call_back": get_delta,
        ...
        descriptors.append(
            create_desc(
                {
                    "name": f"{SSL_NAME_PREFIX}removes_miss",
                    "value_type": "float",
                    "units": "removes/sec",
                    "format": "%.3f",
                    "call_back": get_delta,
